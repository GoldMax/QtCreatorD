...
DRunConfiguration::DRunConfiguration(ProjectExplorer::Target *parent) :
		...
		m_runMode(ApplicationLauncher::Mode::Gui)
{
...
ProjectExplorer::RunConfiguration::ConfigurationState DRunConfiguration::ensureConfigured(QString *errorMessage)
{
	if (isConfigured())
		return validateExecutable(0, errorMessage) ? RunConfiguration::Configured :
																																															RunConfiguration::UnConfigured;
...
ApplicationLauncher::Mode DRunConfiguration::runMode() const
{
	return m_runMode;
}
...
{
	return macroExpander()->expand(m_cmdArguments);
}
...
QList<Core::Id> DRunConfigurationFactory::availableCreationIds(ProjectExplorer::Target *parent,
	...
	: m_ignoreChange(false), m_runConfiguration(rc)
{
	...
	m_workingDirectory->setBaseDirectory(rc->target()->project()->projectDirectory().toString());
...
{
	m_ignoreChange = true;
	m_runConfiguration->setRunMode(on ? ApplicationLauncher::Mode::Console
																																			: ApplicationLauncher::Mode::Gui);
	m_ignoreChange = false;
}
...
void DRunConfigurationWidget::changed()
{
	...
	m_useTerminalCheck->setChecked(m_runConfiguration->runMode()
																																== ApplicationLauncher::Mode::Console);
}
