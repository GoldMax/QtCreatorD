...
{
	//--vvv-- GoldMax --vvv--
	Utils::Id mode = d->runMode;
	RunConfiguration *runConfig = d->runConfiguration;
	this->d = std::make_unique<RunControlPrivate>(this,  mode);
	setRunConfiguration(runConfig);
	createMainWorker();
	d->setState(RunControlState::Initialized);
...
void RunControlPrivate::onWorkerStarted(RunWorker *worker)
{
    ...
    if (state == RunControlState::Starting) {
								showError(worker->d->id + " start succeeded");
        continueStart();
        return;
    }
