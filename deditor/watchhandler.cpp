...
QString WatchModel::formattedValue(const WatchData &data) const
{
    const QString &value = data.value;
				...
				if(data.type == "_Array_char")
				{
					unsigned long len = 0;
					char* ptr = 0;
					const WatchItem& item = static_cast<const WatchItem&>(data);
					if(item.children.length() == 0)
					{
						const QModelIndex ix = m_handler->watchDataIndex(data.iname);
						if(canFetchMore(ix))
							((QAbstractItemModel*)this)->fetchMore(ix);
					}
					foreach(WatchItem* i, item.children)
						if(i->name == QLatin1String("length"))
							len = i->value.toULong();
						else if(i->name == QLatin1String("ptr"))
							ptr = (char*)(*(char**)(i->address));
					if(len > 0)
